...
const generateRandomFunding = () => {
    ...
    const randomRewardSet = rewardNames[Math.floor(Math.random() * rewardNames.length)];
...
export default function FundingCreatePage() {
    ...
    const [form, setForm] = useState({
        ...
        type: 'RESERVED',
        rewards: [
            { name: '', description: '', limitQty: '', badgeType: '', price: '', offerAt: '', shippingCharge: '0' },
        ],
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            openModal({ title: '성공', message: '펀딩이 생성되었습니다!', type: 'success' });
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white border rounded-lg p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium mb-1">목표 금액 (원)</label>
                            <input
                                type="number"
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1">펀딩 타입</label>
                            <select
                                value={form.type}
                ...
                <div className="bg-white border rounded-lg p-6 space-y-4">
                    <div className="flex justify-between items-center border-b pb-2">
                        <h2 className="font-bold text-lg">리워드 ({form.rewards.length}개)</h2>
                        <button
                            type="button"
                            onClick={addReward}
                    ...
                    {form.rewards.map((reward, idx) => (
                        <div key={idx} className="border rounded-lg p-4 space-y-3 bg-gray-50">
                            <div className="flex justify-between items-center">
                                <span className="font-bold text-sm text-gray-600">리워드 #{idx + 1}</span>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-xs font-medium mb-1">리워드명</label>
                                    <input
                                        type="text"
                                        value={reward.name}
                                ...
                                <div>
                                    <label className="block text-xs font-medium mb-1">뱃지 타입</label>
                                    <input
                                        type="text"
                            ...
                            <div className="grid grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input
                                        type="number"
                                        value={reward.price}
                                        onChange={(e) => handleRewardChange(idx, 'price', e.target.value)}
                                        className="w-full border rounded px-3 py-1.5 text-sm"
                                        required
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium mb-1">수량 제한</label>
                                    <input
                                        type="number"
                                        value={reward.limitQty}
                                        onChange={(e) => handleRewardChange(idx, 'limitQty', e.target.value)}
                                        className="w-full border rounded px-3 py-1.5 text-sm"
                                        required
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium mb-1">배송비 (원)</label>
                                    <input
                                        type="number"
                                        value={reward.shippingCharge}
                                        onChange={(e) => handleRewardChange(idx, 'shippingCharge', e.target.value)}
                                        className="w-full border rounded px-3 py-1.5 text-sm"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium mb-1">제공 시작일</label>
                                    <input
                                        type="datetime-local"
                                        value={reward.offerAt}
                                        onChange={(e) => handleRewardChange(idx, 'offerAt', e.target.value)}
                                        className="w-full border rounded px-3 py-1.5 text-sm"
                                        required
                                    />
                                </div>
